...
- **Activities**:
  ...
  - **Never silently delete a test.** When a change removes or replaces an existing test, either convert it in place or leave a `REVIEW PLACEHOLDER (issue #N)` comment mapping the old test title to the new test(s) that cover it (`old title -> new title`), so reviewers can see nothing was lost. Tag it with the issue number; do not tell the reader to delete it.
  - **Placeholders stay in the merged code; clear the older ones instead.** A placeholder for the issue in progress is merged as-is: tests are not shipped with the libraries, so it costs nothing, and deleting it after approval would mean committing on top of a reviewed branch — which invalidates the review. What prevents accumulation is the *next* issue: while extending the suite, delete the placeholders tagged with issues that are already merged and unrelated to the review in progress.
